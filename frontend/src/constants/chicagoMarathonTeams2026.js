 ...
 * on the 2026 Chicago Marathon "Participating Running Clubs" page. Display copy (names,
 * contact person, schedules, link text) lives per-language in
 * `marathonWelcome.teams2026Page.teams` in `content/siteContent.js`, keyed by `key` below.
 *
 * Independent from chicagoMarathonTeams2025.js on purpose: the 2025 roster is an archived
 * record and should never change once published. This currently starts as the same clubs as
 * 2025 (until each club submits their own 2026 photo/video), but edit this file directly as
 * clubs update — never re-derive it from the 2025 file.
 *
 * Order: CHI Running Club always first, then every other club alphabetically by its English
 * club name (see `name` in `teams2026Page.teams` for each key).
 */
export const CHICAGO_MARATHON_TEAMS_2026 = [
  {
    key: 'chirunners',
    flag: '🇺🇸',
    photoUrl:
